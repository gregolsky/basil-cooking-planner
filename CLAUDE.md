...
**Theme photography.** Two tokens, both re-pointed in PRL's `:root` so no surface needs its own `html[data-theme="prl"]` override. `--hero-image` is the tall landing/nav photo (`.home-hero`, PRL's `.nav`). `--page-hero-image` is the short wide band (`.page-hero`) — a separate shot because that band is ~7:1 at 1440px and a slice of the landing photo read badly at that ratio. Its source (`page-hero.webp`) is cropped to just the ingredients so the file carries only what's visible; the band anchors `center top` because the `::after` gradient fades its own bottom out.
