...
- `PageHero.tsx` — the `.page-hero` photo band (themed via `--page-hero-image`, `.no-print`) that opens `/plans`, `/plans/:id`, `/dishes`, `/settings` and `/new-plan`. Rendered as a sibling *before* each page's `.page` wrapper, since it's full-bleed. Deliberately not on `/extend-plan/:id` or `/import` — those are secondary task flows where the band would just push the form down
