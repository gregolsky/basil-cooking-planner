...
export function ViolationsPanel({ plan }: Props) {
  ...
  return (
    <div className="violations-panel">
      <h2>Przekroczone wykluczenia</h2>
      {plan.violations.length === 0 && (
        <div className="muted">Brak naruszeń — plan spełnia wszystkie reguły.</div>
      )}
      {hard.length > 0 && (
        <>
          <h3 style={{ fontSize: '1rem', marginTop: 8 }}>Twarde (wymaga uwagi)</h3>
          {hard.map((v, i) => (
      ...
      {soft.length > 0 && (
        <>
          <h3 style={{ fontSize: '1rem', marginTop: 10 }}>Miękkie (preferencje)</h3>
          {soft.map((v, i) => (
      ...
      {info.length > 0 && (
        <>
          <h3 style={{ fontSize: '1rem', marginTop: 10 }}>Informacje</h3>
          {info.map((v, i) => (
